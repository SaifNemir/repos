...
namespace MedicalServiceSystem
{
    ...
    public partial class FRMreportApproveMedicine : Telerik.WinControls.UI.RadForm
    {
        ...
        private static FRMreportApproveMedicine defaultInstance;

        /// <summary>
        /// Added by the VB.Net to C# Converter to support default instance behavour in C#
        /// </summary>
        public static FRMreportApproveMedicine Default
        {
            get
            {
                if (defaultInstance == null)
                {
                    defaultInstance = new FRMreportApproveMedicine();
        ...
        private void RDDwaCNT_CheckedChanged(object sender, EventArgs e)
        {
            if (RDDwaCNT.Checked == true)
            {
                ...
                {
                    db.Database.CommandTimeout = 0;
                    var GetCent = db.Database.SqlQuery<ReportForAll>("SELECT dbo.MedicineForReclaims.Generic_name AS Row6,convert(decimal, SUM(dbo.ApproveMedicineDetails.Quantity)) AS Row2,convert(decimal, SUM(dbo.ApproveMedicineDetails.ApprovedQuantity))  AS Row3, COUNT(dbo.ApproveMedicines.Id) AS Row1 FROM dbo.ApproveMedicines INNER JOIN dbo.ApproveMedicineDetails ON dbo.ApproveMedicines.Id = dbo.ApproveMedicineDetails.ApproveMedicineId INNER JOIN dbo.MedicineForReclaims ON dbo.ApproveMedicineDetails.ServiceId = dbo.MedicineForReclaims.Id WHERE dbo.ApproveMedicines.ApproveDate BETWEEN '" + d_start.Value + "' and '" + d_end.Value + "' and dbo.ApproveMedicines.RowStatus<>2 GROUP BY dbo.MedicineForReclaims.Generic_name ORDER BY Row1 DESC").ToList();
                    if (GetCent.Count > 0)
                    {
